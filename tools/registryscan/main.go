...
func findRegistryResolution(ctx context.Context, manager *index.IndexManager, packages []cargolock.Package, published time.Time) (*index.RegistryResolution, error) {
	...
	if needsCurrent {
		...
		handle, err := manager.GetRepository(ctx, index.RepositoryKey{Type: index.CurrentIndex})
		if err != nil {
			return nil, fmt.Errorf("failed to get current repository: %v", err)
		}
		defer handle.Close()
	...
	for i := len(relevantSnapshots) - 1; i >= 0; i-- {
		...
		handle, err := manager.GetRepository(ctx, index.RepositoryKey{Type: index.SnapshotIndex, Name: snapshotDate})
		if err != nil {
			fmt.Printf("Warning: failed to get snapshot %s: %v\n", snapshotDate, err)
			continue
		}
		defer handle.Close()
		repos = append(repos, handle.Repository)
