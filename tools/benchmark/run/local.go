...
func NewLocalExecutionService(prebuildURL string, store rebuild.LocatableAssetStore) ExecutionService {
	return &localExecutionService{prebuildURL: prebuildURL, store: store}
}
...
func (s *localExecutionService) RebuildPackage(ctx context.Context, req schema.RebuildPackageRequest) (*schema.Verdict, error) {
	...
	if err := build(ctx, t, strategy, s.store, s.prebuildURL); err != nil {
		verdict.Message = err.Error()
...
func build(ctx context.Context, t rebuild.Target, strategy rebuild.Strategy, out rebuild.LocatableAssetStore, prebuildURL string) error {
	...
	err = template.Must(template.New("").Parse(
		`set -eux
{{.InstallCmd}} curl
curl `+prebuildURL+`/timewarp > timewarp
chmod +x timewarp
./timewarp -port 8081 &
