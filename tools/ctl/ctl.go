...
var runBenchmark = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if *overwriteMode != "" {
			if *overwriteMode != string(schema.OverwriteServiceUpdate) && *overwriteMode != string(schema.OverwriteForce) {
				log.Fatalf("Invalid overwrite-mode: %s. Expected one of 'SERVICE_UPDATE' or 'FORCE'", *overwriteMode)
			}
		}
		var apiURL *url.URL
