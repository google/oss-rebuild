...
func Handler(ctx context.Context, cfg Config, deps *Deps) (*act.NoOutput, error) {
	if cfg.OverwriteMode != "" {
		if cfg.OverwriteMode != string(schema.OverwriteServiceUpdate) && cfg.OverwriteMode != string(schema.OverwriteForce) {
			log.Fatalf("Invalid overwrite-mode: %s. Expected one of 'SERVICE_UPDATE' or 'FORCE'", cfg.OverwriteMode)
		}
	}
