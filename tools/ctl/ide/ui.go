...
func NewTuiApp(dex rundex.Reader, watcher rundex.Watcher, rundexOpts rundex.FetchRebuildOpts, benches benchmark.Repository, buildDefs rebuild.LocatableAssetStore, butler localfiles.Butler, model *genai.GenerativeModel) *TuiApp {
	var t *TuiApp
	{
	...
	if err := cmdReg.AddGlobals(commands.NewGlobalCmds(t.app, t.rb, modalFn, butler, model, buildDefs, dex, benches)...); err != nil {
		log.Fatal(err)
	}
	if err := cmdReg.AddRebuildGroups(commands.NewRebuildGroupCmds(t.app, t.rb, modalFn, butler, model, buildDefs, dex, benches)...); err != nil {
		log.Fatal(err)
	}
	if err := cmdReg.AddRebuilds(commands.NewRebuildCmds(t.app, t.rb, modalFn, butler, model, buildDefs, dex, benches)...); err != nil {
		log.Fatal(err)
	}
