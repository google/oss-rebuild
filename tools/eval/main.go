...
func main() {
	...
	p = p.ParDo(30, func(r rundex.Rebuild, out chan<- rundex.Rebuild) {
		...
		if _, err := cache.Reader(ctx, rebuild.Asset{Target: r.Target(), Type: AttemptAsset}); errors.Is(err, fs.ErrNotExist) {
			...
			if err := enc.Encode(r.SmoketestAttempt); enc != nil {
				panic(err)
			}
		}
		...
		if _, err := cache.Reader(ctx, rebuild.Asset{Target: r.Target(), Type: rebuild.DebugLogsAsset}); errors.Is(err, fs.ErrNotExist) {
			if err := rebuild.AssetCopy(ctx, cache, debug, rebuild.Asset{Target: r.Target(), Type: rebuild.DebugLogsAsset}); err != nil {
				panic(err)
			}
		}
		out <- r
	...
	sp := pipe.ParInto(concurrency, p, func(r rundex.Rebuild, out chan<- Recovery) {
		...
		if ns, ok := s.(*npm.NPMPackBuild); ok {
			commands = []string{"npm pack"}
			npmVersion = ns.NPMVersion
		...
		if inst.Location.Dir != "." {
			newCommands = append(newCommands, "cd "+inst.Location.Dir)
		}
		...
		strategy := rebuild.ManualStrategy{
			...
			Deps: must(rebuild.PopulateTemplate(`/usr/bin/npm config --location-global set registry {{.BuildEnv.TimewarpURL "npm" .RegistryTime}}
...
`, struct {
				...
				BuildEnv:     rebuild.BuildEnv{TimewarpHost: "localhost:8081"},
				NodeVersion:  must(reg.Version(ctx, r.Package, r.Version)).NodeVersion,
				RegistryTime: must(reg.Package(ctx, r.Package)).UploadTimes[r.Version],
			})),
			Build:      fmt.Sprintf("/usr/local/bin/npx -p npm@%s -c '%s'", npmVersion, strings.Join(newCommands, " && ")),
			OutputPath: path.Join(inst.Location.Dir, r.Target().Artifact),
		}
		out <- Recovery{
			Rebuild:          r,
