...
func rebuildPackage(ctx context.Context, req schema.RebuildPackageRequest, deps *RebuildPackageDeps) (*schema.Verdict, error) {
	...
	if exists, err := a.BundleExists(ctx, t); err != nil {
		...
		case schema.OverwriteServiceUpdate:
			...
			verifier, err := dsse.NewEnvelopeVerifier()
			if err != nil {
				v.Message = errors.Wrap(err, "creating envelope verifier").Error()
				return &v, nil
			}
			...
			if err != nil {
				v.Message = api.AsStatus(codes.FailedPrecondition, errors.Wrap(err, "overwrite denied: no rebuild attestation in bundle")).Error()
				return &v, nil
			}
		...
		deps.OverwriteAttestations = true
	} else {
		deps.OverwriteAttestations = true
	}
