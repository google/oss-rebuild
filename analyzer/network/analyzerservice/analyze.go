...
func createAndPublishAttestations(ctx context.Context, deps *AnalyzerDeps, mux rebuild.RegistryMux, t rebuild.Target, strategy rebuild.Strategy, obID string, rebuildAttestation *attestation.RebuildAttestation) error {
	...
	networkStmt, eqStmt, err := createNetworkAttestations(ctx, t, strategy, obID, deps.ServiceRepo, deps.OutputAnalysisStore, deps.InputAttestationStore, rebuildAttestation, *rebuiltSummary, *upstreamSummary)
	if err != nil {
...
func createNetworkAttestations(ctx context.Context, t rebuild.Target, strategy rebuild.Strategy, obID string, serviceLoc rebuild.Location, analysisStore rebuild.LocatableAssetStore, inputAttestationStore rebuild.AssetStore, rebuildAttestation *attestation.RebuildAttestation, rebuiltSummary, upstreamSummary verifier.ArtifactSummary) (network, equivalence *in_toto.ProvenanceStatementSLSA1, err error) {
