...
func analysisExists(ctx context.Context, store rebuild.LocatableAssetStore, t rebuild.Target) (bool, error) {
...
func createAndPublishAttestations(ctx context.Context, deps *AnalyzerDeps, mux rebuild.RegistryMux, t rebuild.Target, strategy rebuild.Strategy, id string, rebuildAttestation *attestation.RebuildAttestation) error {
	remoteStore, err := deps.RemoteMetadataStoreBuilder(ctx, id)
	if err != nil {
	...
	networkStmt, eqStmt, err := createNetworkAttestations(ctx, t, strategy, id, deps.ServiceRepo, deps.OutputAnalysisStore, deps.InputAttestationStore, rebuildAttestation, *rebuiltSummary, *upstreamSummary)
	if err != nil {
	...
	err = publishNetworkBundle(ctx, deps.OutputAnalysisStore, signer, t, networkStmt, eqStmt)
	if err != nil {
...
func createNetworkAttestations(ctx context.Context, t rebuild.Target, strategy rebuild.Strategy, id string, serviceLoc rebuild.Location, analysisStore rebuild.LocatableAssetStore, inputAttestationStore rebuild.AssetStore, rebuildAttestation *attestation.RebuildAttestation, rebuiltSummary, upstreamSummary verifier.ArtifactSummary) (network, equivalence *in_toto.ProvenanceStatementSLSA1, err error) {
	...
	metadata := slsa1.BuildMetadata{InvocationID: id}
		...
		Predicate: attestation.ArtifactEquivalencePredicate{
			BuildDefinition: attestation.ArtifactEquivalenceBuildDef{
				...
				ExternalParameters: attestation.ArtifactEquivalenceParams{
					Candidate: rebuiltSummary.URI,
					Target:    upstreamSummary.URI,
				},
				InternalParameters: internalParams,
				ResolvedDependencies: attestation.ArtifactEquivalenceDeps{
					RebuiltArtifact:  slsa1.ResourceDescriptor{Name: rebuiltSummary.URI, Digest: rebuiltDigests},
					UpstreamArtifact: slsa1.ResourceDescriptor{Name: upstreamSummary.URI, Digest: upstreamDigests},
				},
			},
			RunDetails: attestation.ArtifactEquivalenceRunDetails{
				...
				Byproducts: attestation.ArtifactEquivalenceByproducts{
					NormalizedArtifact: slsa1.ResourceDescriptor{Name: rebuiltSummary.URI, Digest: rebuiltDigests},
				},
			},
		},
	}).ToStatement()
...
func publishNetworkBundle(ctx context.Context, store rebuild.LocatableAssetStore, signer verifier.InTotoEnvelopeSigner, t rebuild.Target, statements ...*in_toto.ProvenanceStatementSLSA1) error {
