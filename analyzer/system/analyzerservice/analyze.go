...
func analysisExists(ctx context.Context, store rebuild.AssetStore, t rebuild.Target) (bool, error) {
	_, err := store.Reader(ctx, SystemAnalysisAsset.For(t))
...
func createAndPublishAttestations(ctx context.Context, deps *AnalyzerDeps, mux rebuild.RegistryMux, t rebuild.Target, strategy rebuild.Strategy, obID string, rebuildAttestation *attestation.RebuildAttestation) error {
	...
	err = copySystemLog(ctx, remoteStore, deps.OutputAnalysisStore, t)
	if err != nil {
		return errors.Wrap(err, "copying system log")
	}
	...
	// Create system trace attestations
	systemStmt, eqStmt, err := createSystemTraceAttestations(ctx, t, strategy, obID, deps.ServiceRepo, deps.OutputAnalysisStore, deps.InputAttestationStore, deps.LocalMetadataStore, rebuildAttestation, *rebuiltSummary, *upstreamSummary)
	if err != nil {
		return errors.Wrap(err, "creating system trace attestations")
	}
	// Publish bundle with both attestations
	signer := verifier.InTotoEnvelopeSigner{EnvelopeSigner: deps.Signer}
	err = publishSystemTraceBundle(ctx, deps.OutputAnalysisStore, signer, t, eqStmt, systemStmt)
	if err != nil {
...
func copySystemLog(ctx context.Context, remoteStore, analysisStore rebuild.LocatableAssetStore, t rebuild.Target) error {
	systemlogReader, err := remoteStore.Reader(ctx, rebuild.TetragonLogAsset.For(t))
	if err != nil {
		return errors.Wrap(err, "reading system log from remote store")
	}
	defer systemlogReader.Close()
	systemlogWriter, err := analysisStore.Writer(ctx, SystemTraceAsset.For(t))
	if err != nil {
		return errors.Wrap(err, "creating system log writer")
	}
	defer systemlogWriter.Close()
	_, err = io.Copy(systemlogWriter, systemlogReader)
	return err
}

func createSystemTraceAttestations(ctx context.Context, t rebuild.Target, strategy rebuild.Strategy, obID string, serviceLoc rebuild.Location, analysisStore rebuild.LocatableAssetStore, inputAttestationStore rebuild.AssetStore, metadataStore rebuild.AssetStore, rebuildAttestation *attestation.RebuildAttestation, rebuiltSummary, upstreamSummary verifier.ArtifactSummary) (systemTrace, equivalence *in_toto.ProvenanceStatementSLSA1, err error) {
	subjectDigest := rebuildAttestation.Subject[0].Digest
	// Calculate system log digest
	systemlogReader, err := analysisStore.Reader(ctx, SystemTraceAsset.For(t))
	if err != nil {
		return nil, nil, errors.Wrap(err, "reading system log for digest")
	}
	defer systemlogReader.Close()
	hasher := hashext.NewTypedHash(crypto.SHA256)
	if _, err := io.Copy(hasher, systemlogReader); err != nil {
		return nil, nil, errors.Wrap(err, "hashing system log")
	}
	systemlogURL := analysisStore.URL(SystemTraceAsset.For(t))
	systemlogDescriptor := slsa1.ResourceDescriptor{
		Name: systemlogURL.String(),
	...
	var deps SystemTraceRebuildDeps
	...
	// Create system trace attestation
	systemTrace, err = (&SystemTraceRebuildAttestation{
		...
		Predicate: SystemTraceRebuildPredicate{
			BuildDefinition: SystemTraceRebuildBuildDef{
				BuildType: BuildTypeSystemTraceRebuildV01,
				ExternalParameters: SystemTraceRebuildParams{
			...
			RunDetails: SystemTraceRebuildRunDetails{
				Builder:       builder,
				BuildMetadata: metadata,
				Byproducts: SystemTraceRebuildByproducts{
					SystemLog:     systemlogDescriptor,
	...
	if err != nil {
		return nil, nil, errors.Wrap(err, "creating system trace attestation")
	}
	...
	return systemTrace, equivalence, nil
}

func publishSystemTraceBundle(ctx context.Context, store rebuild.AssetStore, signer verifier.InTotoEnvelopeSigner, t rebuild.Target, statements ...*in_toto.ProvenanceStatementSLSA1) error {
	w, err := store.Writer(ctx, SystemAnalysisAsset.For(t))
	if err != nil {
