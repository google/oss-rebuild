...
func TestParseDebianArtifact(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if source := actual.Version.RealUpstream(); source != tc.expectedRealUpstream {
				t.Errorf("RealUpstream mismatch: got %v, want %v", source, tc.expectedRealUpstream)
			}
