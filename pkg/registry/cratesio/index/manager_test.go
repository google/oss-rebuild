...
`
	...
	if _, err := tempFS.Stat("snapshots/2024-02-01"); err != nil {
		t.Error("2024-02-01 should exist on filesystem")
	}
	if _, err := tempFS.Stat("snapshots/2024-03-01"); err != nil {
...
`
	...
	must1((&SnapshotIndexFetcher{Date: "2024-01-01"}).Fetch(context.Background(), must(fs.Chroot("snapshots/2024-01-01"))))

	snapshot2Path := filepath.Join(tempDir, "snapshots", "2024-02-01")
	must1(fs.MkdirAll(snapshot2Path, 0755))
	must1((&SnapshotIndexFetcher{Date: "2024-02-01"}).Fetch(context.Background(), must(fs.Chroot("snapshots/2024-02-01"))))
