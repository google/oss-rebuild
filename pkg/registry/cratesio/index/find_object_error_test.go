...
func TestFindRegistryResolutionPropagatesTailObjectError(t *testing.T) {
	storage := memory.NewStorage()
	repo, err := gitxtest.CreateRepoFromYAML(`commits:
  - id: initial
    files:
...
`, &gitxtest.RepositoryOptions{Storer: storage})
	...
	initial, err := repo.CommitObject(repo.Commits["initial"])
	if err != nil {
		t.Fatal(err)
	}
	delete(storage.Objects, initial.TreeHash)
