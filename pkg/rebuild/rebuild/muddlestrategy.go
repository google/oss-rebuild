...
// MuddleStep represents a single step in the build process.
type MuddleStep struct {
...
// MuddleStrategy allows use of composable steps to define the build.
type MuddleStrategy struct {
	Location
	Source     []MuddleStep `json:"src" yaml:"src,omitempty"`
	Deps       []MuddleStep `json:"deps" yaml:"deps,omitempty"`
	Build      []MuddleStep `json:"build" yaml:"build,omitempty"`
...
var _ Strategy = &MuddleStrategy{}

// GenerateFor generates the instructions for a MuddleStrategy.
func (s *MuddleStrategy) GenerateFor(t Target, be BuildEnv) (Instructions, error) {
...
// command defines a command with its system requirements.
type command struct {
	Script string
	Needs  []string // Required system deps
}

func (cmd command) Join(other command) command {
	return command{
		Script: strings.Join([]string{cmd.Script, other.Script}, "\n"),
		Needs:  append(cmd.Needs, other.Needs...),
	}
}

// generateForSteps processes a slice of MuddleSteps and returns a combined script
func (s *MuddleStrategy) generateForSteps(steps []MuddleStep, t Target, be BuildEnv) (command, error) {
	var ret command
...
func (s *MuddleStrategy) generateForStep(step MuddleStep, t Target, be BuildEnv) (command, error) {
	if (step.Runs == "") == (step.Uses == "") {
		return command{}, errors.New("exactly one of 'runs' or 'uses' must be provided")
	}
	if step.Runs != "" {
		return command{Script: step.Runs}, nil
	}
	cmd, ok := builtinCommands[step.Uses]
	if !ok {
		return command{}, errors.Errorf("unknown 'uses' command: %s", step.Uses)
	}
	buf := &bytes.Buffer{}
	data := struct {
	...
	err := cmd.Template.Execute(buf, data)
	if err != nil {
		return command{}, errors.Wrap(err, "executing template")
	}
	return command{Script: buf.String(), Needs: cmd.Needs}, nil
}

// templateCommand defines a command template with its system requirements.
type templateCommand struct {
	Template *template.Template
	Needs    []string // Required system deps
}

var builtinCommands = map[string]*templateCommand{
	"git-checkout": {
