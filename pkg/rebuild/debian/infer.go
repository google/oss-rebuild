...
var versionExtractRegex = regexp.MustCompile(`^(?P<name>[^_]+)_(?P<nonbinary_version>[^_+]+)(?P<binary_version>\+.*)?_(?P<arch>[^_]+)\.deb$`)
...
func (Rebuilder) InferStrategy(ctx context.Context, t rebuild.Target, mux rebuild.RegistryMux, rcfg *rebuild.RepoConfig, hint rebuild.Strategy) (rebuild.Strategy, error) {
	...
	var infoURL string
	if matches := versionExtractRegex.FindStringSubmatch(t.Artifact); matches != nil {
		nbversion := matches[versionExtractRegex.SubexpIndex("nonbinary_version")]
		bversion := matches[versionExtractRegex.SubexpIndex("binary_version")]
		version := nbversion
		if !strings.HasPrefix(bversion, "+deb") {
			version += bversion
		}
		arch := matches[versionExtractRegex.SubexpIndex("arch")]
		infoURL = debreg.BuildInfoURL(name, version, arch)
	} else {
