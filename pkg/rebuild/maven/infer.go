...
func inferBuildTool(commit *object.Commit) (string, error) {
	...
	fileIter.ForEach(func(f *object.File) error {
		...
		if currentBuildTool != "" {
			if minDepth == -1 || currentDepth < minDepth {
				...
				bestBuildTool = currentBuildTool
			}
		}
	...
	return "", errors.Errorf("only Maven is supported, but no pom.xml found")
}
