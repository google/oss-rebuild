...
func inferBuildSystem(commit *object.Commit) (rebuild.StrategyInferer, error) {
	var bestInferer rebuild.StrategyInferer
	minDepth := -1

	fileIter, _ := commit.Files()
	fileIter.ForEach(func(f *object.File) error {
		var currentInferer rebuild.StrategyInferer
		var currentDepth int

		// Check for Maven's build file
		if path.Base(f.Name) == "pom.xml" && !strings.HasPrefix(f.Name, "src/") && !strings.Contains(f.Name, "/src/") {
			currentInferer = &MavenBuildInferer{}
			currentDepth = strings.Count(f.Name, "/")
		}

		// Check for Gradle's build file
		if path.Base(f.Name) == "gradlew" {
			currentInferer = &GradleBuildInferer{}
			currentDepth = strings.Count(f.Name, "/")
		}

		// If a supported build file was found, compare its depth to the current minimum
		if currentInferer != nil {
			if minDepth == -1 || currentDepth < minDepth {
				minDepth = currentDepth
				bestInferer = currentInferer
			}
		}
	...
	if bestInferer != nil {
		return bestInferer, nil
	}

	return nil, errors.Errorf("neither Maven nor Gradle supported build files were found")
}
