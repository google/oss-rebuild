...
func TestStabilizePypi(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expected: []*archive.ZipEntry{
				{FileHeader: &zip.FileHeader{Name: "dist-info/metadata.json", Modified: time.UnixMilli(0)}, Body: []byte("This needed to change (metadata)")},
			},
		...
		},
		{
			...
			expected: []*archive.ZipEntry{
				{FileHeader: &zip.FileHeader{Name: "dist-info/METADATA", Modified: time.UnixMilli(0)}, Body: []byte("Author: Fake Author\nAuthor-Email: fake@gmail.com\nClassifier: License :: OSI Approved :: MIT License\nClassifier: Operating System :: OS Independent\nClassifier: Programming Language :: Python :: 3\nDescription-Content-Type: text/markdown\nHome-Page: https://github.com/fake/testProject\nLicense: UNKNOWN\nMetadata-Version: 2.1\nName: testProject\nPlatform: UNKNOWN\nRequires-Dist: distlib\nRequires-Dist: pandas\nRequires-Dist: pyarrow\nRequires-Dist: requests\nRequires-Dist: urllib3 (>=1.26.3)\nRequires-Dist: xmltodict\nRequires-Dist: zipfile36\nRequires-Python: >=3.6\nSummary: A simple test project\nVersion: 0.3.0\nMessageBodyDescription: # Pandas Read XML\nMessageBodyDescription: \nMessageBodyDescription: A tool to help read XML files as pandas dataframes.\nMessageBodyDescription: \nMessageBodyDescription: See example in [Google Colab here](https://colab.research.google.com/github/minchulkim87/pandas_read_xml/blob/master/pandas_read_xml_example.ipynb)\nMessageBodyDescription: \nMessageBodyDescription: Isn't it annoying working with data in XML format? I think so. Take a look at this simple example.\nMessageBodyDescription: \nMessageBodyDescription: ```xml\nMessageBodyDescription: <first-tag>\nMessageBodyDescription:     <not-interested>\nMessageBodyDescription:         blah blah\nMessageBodyDescription:     </not-interested>\nMessageBodyDescription:     <second-tag>\nMessageBodyDescription:         <the-tag-you-want-as-root>\nMessageBodyDescription:             <row>\nMessageBodyDescription:                 <columnA>\nMessageBodyDescription:                     The data that you want\nMessageBodyDescription:                 </columnA>\nMessageBodyDescription:                 <columnB>\nMessageBodyDescription:                     More data that you want\nMessageBodyDescription:                 </columnB>\nMessageBodyDescription:             </row>\nMessageBodyDescription:             <row>\nMessageBodyDescription:                 <columnA>\nMessageBodyDescription:                     Yet more data that you want\nMessageBodyDescription:                 </columnA>\nMessageBodyDescription:                 <columnB>\nMessageBodyDescription:                     Eh, get this data too\nMessageBodyDescription:                 </columnB>\nMessageBodyDescription:             </row>\nMessageBodyDescription:         </the-tag-you-want-as-root>\nMessageBodyDescription:     </second-tag>\nMessageBodyDescription:     <another-irrelevant-tag>\nMessageBodyDescription:         some other info that you do not want\nMessageBodyDescription:     </another-irrelevant-tag>\nMessageBodyDescription: </first-tag>\nMessageBodyDescription: ```\nMessageBodyDescription: \nMessageBodyDescription: I wish there was a simple `df = pd.read_xml('some_file.xml')` like `pd.read_csv()` and `pd.read_json()` that we all love.\nMessageBodyDescription: \nMessageBodyDescription: I can't solve this with my time and skills, but perhaps this package will help get you started.\nMessageBodyDescription: \nMessageBodyDescription: \nMessageBodyDescription: ## Install\nMessageBodyDescription: \nMessageBodyDescription: ```bash\nMessageBodyDescription: pip install pandas_read_xml\nMessageBodyDescription: ```\nMessageBodyDescription: \nMessageBodyDescription: ## Import package\nMessageBodyDescription: \nMessageBodyDescription: ```python\nMessageBodyDescription: import pandas_read_xml as pdx\nMessageBodyDescription: ```\nMessageBodyDescription: \nMessageBodyDescription: ## Read XML as pandas dataframe\nMessageBodyDescription: \nMessageBodyDescription: You will need to identify the path to the \"root\" tag in the XML from which you want to extract the data.\nMessageBodyDescription: \nMessageBodyDescription: ```python\nMessageBodyDescription: df = pdx.read_xml(\"test.xml\", ['first-tag', 'second-tag', 'the-tag-you-want-as-root'])\nMessageBodyDescription: ```\nMessageBodyDescription: \nMessageBodyDescription: By default, pandas-read-xml will treat the root tag as being the \"rows\" of the pandas dataframe. If this is not true, pass the argument `root_is_rows=False`.\nMessageBodyDescription: \nMessageBodyDescription: *Sometimes, the XML structure is such that pandas will treat rows vs columns in a way that we think are opposites. For these cases, the read_xml may fail. Try using `transpose=True` as an argument in such cases. This argument will only affect the reading if `root_is_rows=False` is passed.\nMessageBodyDescription: \nMessageBodyDescription: # Auto Flatten\nMessageBodyDescription: \nMessageBodyDescription: The real cumbersome part of working with XML data (or JSON data) is that they do not represent a single table. Rather, they are a (nested) tree representations of what probably were relational databases. Often, these XML data are exported without a clearly documented schema, and more often, no clear way of navigating the data.\nMessageBodyDescription: \nMessageBodyDescription: What is even more annoying is that, in comparison to JSON, the data structures are not consistent across XML files from the same schema. Some files may have multiples of the same tag, resulting in a list-type data, while in other files of the *same* schema will only have on of that tag, resulting in a non-list-type data. In other times, the tags are not present which means that the resulting \"column\" is not just null, but not even a column. This makes it difficult to \"flatten\".\nMessageBodyDescription: \nMessageBodyDescription: Pandas already has some tools to help \"explode\" (items in list become separate rows) and \"normalise\" (key, value pairs in one column become separate columns of data), but they fail when there are these mixed types within the same tags (columns). Besides, \"flattening\" (combining exploding and normalising) duplicates other data in the dataframe as well, leading to an explosion of memory requirements.\nMessageBodyDescription: \nMessageBodyDescription: So, in this tool, I have also attempted to make a few different tools to separate the relational tables.\nMessageBodyDescription: \nMessageBodyDescription: See the example in Colab (or run the notebook elsewhere)\nMessageBodyDescription: \nMessageBodyDescription: The `auto_separate_tables` method will separate out what it guesses to be separate tables. The resulting `data` is a dictionary where the keys are the \"table names\" and the corresponding values are the pandas dataframes. Each of the separate tables will have the `key_columns` as common columns.\nMessageBodyDescription: \nMessageBodyDescription: You can see the list of separated tables by using python dictionary methods.\nMessageBodyDescription: \nMessageBodyDescription: ```python\nMessageBodyDescription: data.keys()\nMessageBodyDescription: ```\nMessageBodyDescription: \nMessageBodyDescription: And then view the table of interest.\nMessageBodyDescription: \nMessageBodyDescription: There are also other \"smaller\" functions that does parts of the job:\nMessageBodyDescription: \nMessageBodyDescription: - flatten(df)\nMessageBodyDescription: - auto_flatten(df, key_columns)\nMessageBodyDescription: - fully_flatten(df, key_columns)\nMessageBodyDescription: \nMessageBodyDescription: Even more if you look through the code.\nMessageBodyDescription: \nMessageBodyDescription: \nMessageBodyDescription: \n\n")},
			},
			pypiStabilizers: []Stabilizer{StableWheelBuildMetadata},
		},
		// TODO - Need another here to test the other kind of description header for stablebuildwheel above
		{
			...
			expected: []*archive.ZipEntry{
				{FileHeader: &zip.FileHeader{Name: "dist-info/DESCRIPTION.rst", Modified: time.UnixMilli(0)}, Body: []byte("This needed to change (description)")},
			},
