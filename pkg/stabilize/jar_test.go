...
func TestStableMavenPomProperties(t *testing.T) {
	const pomPath = "META-INF/maven/com.example/foo/pom.properties"
	for _, tc := range []struct {
		...
		{
			test:     "strips_generated_and_date_comments",
			input:    []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: pomPath}, Body: []byte("#Generated by Maven\n#Wed Feb 21 15:54:16 IST 2018\nversion=1.0.0\ngroupId=com.example\nartifactId=foo\n")}},
			expected: []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: pomPath}, Body: []byte("version=1.0.0\ngroupId=com.example\nartifactId=foo\n")}},
		},
		{
		...
		{
			test:     "ignores_pom_properties_outside_META_INF_maven",
			input:    []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: "classes/pom.properties"}, Body: []byte("#Generated by Maven\n#Wed Feb 21 15:54:16 IST 2018\nversion=1.0.0\n")}},
			expected: []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: "classes/pom.properties"}, Body: []byte("#Generated by Maven\n#Wed Feb 21 15:54:16 IST 2018\nversion=1.0.0\n")}},
		},
		{
			test:     "ignores_non_pom_properties_files",
			input:    []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: "META-INF/maven/com.example/foo/other.properties"}, Body: []byte("#A comment\nkey=value\n")}},
			expected: []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: "META-INF/maven/com.example/foo/other.properties"}, Body: []byte("#A comment\nkey=value\n")}},
		},
		{
			test:     "no_leading_comments_is_a_noop",
			input:    []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: pomPath}, Body: []byte("version=1.0.0\ngroupId=com.example\n")}},
			expected: []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: pomPath}, Body: []byte("version=1.0.0\ngroupId=com.example\n")}},
		},
		{
			test:     "leaves_unexpected_leading_comments_alone",
			input:    []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: pomPath}, Body: []byte("#Some other comment\n#Another\nversion=1.0.0\n")}},
			expected: []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: pomPath}, Body: []byte("#Some other comment\n#Another\nversion=1.0.0\n")}},
		},
		{
			test:     "leaves_generated_header_without_date_alone",
			input:    []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: pomPath}, Body: []byte("#Generated by Maven\nversion=1.0.0\n")}},
			expected: []*archive.ZipEntry{{FileHeader: &zip.FileHeader{Name: pomPath}, Body: []byte("#Generated by Maven\nversion=1.0.0\n")}},
		},
	} {
		t.Run(tc.test, func(t *testing.T) {
			var input bytes.Buffer
			...
			var output bytes.Buffer
			zr := must(zip.NewReader(bytes.NewReader(input.Bytes()), int64(input.Len())))
			err := StabilizeZip(zr, zip.NewWriter(&output), NewContext(archive.ZipFormat).WithStabilizers([]Stabilizer{StableMavenPomProperties}))
			if err != nil {
				t.Fatalf("StabilizeZip(%v) = %v, want nil", tc.test, err)
			}
