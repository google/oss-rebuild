...
func TestCustomConfigOneOf_Format(t *testing.T) {
	tests := []struct {
		name string
		str  string
		want *Format
	}{
		{
			name: "tgz format",
			str:  "tgz",
			want: func() *Format { f := TarGzFormat; return &f }(),
		},
		{
			name: "tar format",
			str:  "tar",
			want: func() *Format { f := TarFormat; return &f }(),
		},
		{
			name: "zip format",
			str:  "zip",
			want: func() *Format { f := ZipFormat; return &f }(),
		},
		{
			name: "empty format",
			str:  "",
		...
		},
		{
			name: "unknown format",
			str:  "unknown",
			want: func() *Format { f := UnknownFormat; return &f }(),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want == nil {
				if got != nil {
					t.Errorf("Format() = %v, want nil", *got)
				}
			} else {
				if got == nil {
					t.Errorf("Format() = nil, want %v", *tt.want)
				} else if *got != *tt.want {
...
func TestCreateCustomStabilizers(t *testing.T) {
			...
			configs: []CustomConfigOneOf{
				{
					...
					Reason:    "test reason",
					FormatStr: "unknown", // This will cause an error in Stabilizer()
				},
			},
			defaultFormat: TarFormat,
			wantErr:       true,
			...
			configs: []CustomConfigOneOf{
				{
					...
					Reason: "test reason",
					// No FormatStr, should use default
				},
			},
			defaultFormat: ZipFormat,
			wantLen:       1,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			stabilizers, err := CreateCustomStabilizers(tt.configs, tt.defaultFormat)
			if tt.wantErr {
				if err == nil {
