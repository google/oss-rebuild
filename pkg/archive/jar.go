...
import (
	...
	"github.com/google/oss-rebuild/pkg/archive/manifest"
)
	...
	Func: func(zr *MutableZipReader) {
		...
		for _, zf := range zr.File {
			if strings.HasSuffix(zf.Name, "pom.xml") {
				...
				mavenProject := manifest.ParsePom(content)
				gitPropertiesFiles = append(gitPropertiesFiles, getAllGitPropertiesFileInPom(mavenProject)...)
			}
		}
...
func getAllGitPropertiesFileInPom(mavenProject manifest.Project) []string {
...
func getGitPropertiesFilename(build manifest.Build) []string {
	var gitPropertiesFiles []string
	for _, plugin := range build.Plugins.Plugin {
		if plugin.ArtifactID == "git-commit-id-plugin" && plugin.GroupID == "pl.project13.maven" {
			gitPropertiesFiles = append(gitPropertiesFiles, manifest.ParsePomProperties(plugin.Configuration.GenerateGitPropertiesFilename))
		}
	}
	return gitPropertiesFiles
}
