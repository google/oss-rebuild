...
func TestGCBPlannerSavePostBuildContainer(t *testing.T) {
	...
	foundExportStep := false
	for _, step := range plan.Steps {
		if step.Script != "" && strings.Contains(step.Script, "docker export") {
			foundExportStep = true
		}
	}
	if !foundExportStep {
		t.Error("Expected export step not found when SavePostBuildContainer is true")
	}
}
